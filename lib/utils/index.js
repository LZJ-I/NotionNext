...
import { rewriteStaticAssetUrl } from './jsdMirror'
...
export function loadExternalResource(url, type = 'js') {
  const resourceUrl = rewriteStaticAssetUrl(url)
  // 检查是否已存在
  const elements =
    type === 'js'
      ? document.querySelectorAll(`[src='${resourceUrl}']`)
      : document.querySelectorAll(`[href='${resourceUrl}']`)

  return new Promise((resolve, reject) => {
    if (elements.length > 0 || !resourceUrl) {
      resolve(resourceUrl)
      return resourceUrl
    }
    ...
    if (tag) {
      ...
      tag.onerror = () => {
        console.warn('Load Error', resourceUrl)
        reject(resourceUrl)
      }
