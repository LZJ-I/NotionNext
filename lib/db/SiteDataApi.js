...
import { getPageContentText } from './notion/getPageContentText'
...
const TOTAL_WORD_COUNT_BATCH_SIZE = 5

async function calculatePostWordCount(post, from) {
  if (!post?.id) return 0

  try {
    const rawBlockMap = await fetchNotionPageBlocks(post.id, from)
    const blockMap = adapterNotionBlockMap(rawBlockMap)

    if (!blockMap?.block) return 0

    const formattedBlockMap = {
      ...blockMap,
      block: formatNotionBlock(blockMap.block)
    }
    const content = formattedBlockMap.block?.[post.id]?.value?.content

    if (!content?.length) return 0

    const pageContentText = getPageContentText(
      { id: post.id, content, password: post.password },
      formattedBlockMap
    )

    return countWords(pageContentText).wordCount
  } catch (error) {
    console.warn('[calculatePostWordCount] failed:', post.id, error)
    return 0
  }
}

async function calculateTotalWordCount(posts, from) {
  let totalWordCount = 0

  for (let i = 0; i < posts.length; i += TOTAL_WORD_COUNT_BATCH_SIZE) {
    const batch = posts.slice(i, i + TOTAL_WORD_COUNT_BATCH_SIZE)
    const counts = await Promise.all(
      batch.map(post => calculatePostWordCount(post, `${from}-total-word-count`))
    )
...
async function getCachedTotalWordCount(pageId, posts, from) {
  const cachedData = await getOrSetDataWithCache(
    `site_total_word_count_${pageId}`,
    async (targetPosts, targetFrom) => ({
      totalWordCount: await calculateTotalWordCount(targetPosts, targetFrom)
    }),
    posts,
    from
  )

  return cachedData?.totalWordCount ?? 0
}
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  const totalWordCount = await getCachedTotalWordCount(
    pageId,
    allPages.filter(post => post?.type === 'Post' && post.status === 'Published'),
    from || 'site-data'
  )
